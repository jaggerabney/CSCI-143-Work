...
public class TileListener extends MouseInputAdapter {
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (current == null) {
            if (deltaX != 0 && deltaY != 0) {
                Tile nextTile = new Tile(firstSpot.x, firstSpot.y, deltaX, deltaY, parent.getColor());
                tiles.insertBack(nextTile);
            }
        } else {
            current.translate(newSpot.x - firstSpot.x, newSpot.y - firstSpot.y);
        }
        parent.repaint();
    }
    ...
    private void drawBorder(Graphics g) {
        ...
        int deltaY = lastSpot.y - firstSpot.y;
        if (current != null) {
            g.drawRect(current.getX() + deltaX, current.getY() + deltaY, current.getWidth(), current.getHeight());
        } else {
            int cornerX, cornerY;
            if (deltaX < 0) {
